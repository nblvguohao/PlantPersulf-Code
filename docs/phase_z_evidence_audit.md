...
> **2026-08-14 更正(见 `docs/cross_species_v2_findings_2026-08-14.md` §3)**:
> 加入番茄第 4 物种并补做模型置信度对照(`plddt_context_test` /
> `filter_rows_by_plddt`)后,本节"方向 3/3 一致、结构层趋同"的结论
> **不再成立**。持久化 Cys 与其他 Cys 在 AlphaFold pLDDT 上系统不同
> (水稻/稻瘟菌阳性更高置信、番茄阳性更低置信,均 p≤0.005),限制在
> pLDDT≥70 的残基后,四个物种的 SASA 差异全部不显著。此前的"更包埋"
> 是模型置信度分层的伴随效应,不是独立的结构机制证据。以下两段(原始
> 分析)保留作历史记录,**不得再引用为结论**;对外表述以 v2 更正版为准。

**如实结论(不变,后被 2026-08-14 更正,见上)**:**方向 3/3 一致**(全部"更包埋"),**显著性
